...
namespace ne::raw {
    ...
    SDLWindow::SDLWindow() {
        ...
        if (m_pWindow == nullptr)
            throw SDLException("Cannot create a SDL window.");
    }

    SDLWindow::SDLWindow(const std::string &title, const Integer x,
                         const Integer y, const Integer width,
    ...
    SDLWindow::SDLWindow(const std::string &title, const Integer x,
                         const Integer y, const Integer width,
                         const Integer height, const Uint32 flags) {
    ...
    SDLWindow::SDLWindow(const SDLWindow &lhs) {
        ...
        if (m_pWindow == nullptr)
            throw SDLException("Cannot copy a SDL window.");
    }
    ...
    auto SDLWindow::GetSize() const->ne::math::Vector2D {
        if (!IsVaild())
            throw SDLException(
                "Cannot get window size, SDL window is invaild.");
    ...
    auto SDLWindow::GetWidth() const->Integer {
        if (!IsVaild())
            throw SDLException(
                "Cannot get window width, SDL window is invaild.");
    ...
    auto SDLWindow::GetHeight() const->Integer {
        if (!IsVaild())
            throw SDLException(
                "Cannot get window height, SDL window is invaild.");
    ...
    auto SDLWindow::GetTitle() const->std::string {
        if (!IsVaild())
            throw SDLException(
                "Cannot get window title, SDL window is invaild.");
    ...
    auto SDLWindow::GetFlags() const->UInt32 {
        if (!IsVaild())
            throw SDLException(
                "Cannot get window flags, SDL window is invaild.");
    ...
    void SDLWindow::SetSize(const ne::math::Vector2D &size) {
        if (!IsVaild())
            throw SDLException(
                "Cannot set window size, SDL window is invaild.");
    ...
    void SDLWindow::SetWidth(const Integer width) {
        if (!IsVaild())
            throw SDLException(
                "Cannot set window width, SDL window is invaild.");
    ...
    void SDLWindow::SetHeight(const Integer height) {
        if (!IsVaild())
            throw SDLException(
                "Cannot set window height, SDL window is invaild.");
    ...
    void SDLWindow::SetTitle(const std::string &title) {
        if (!IsVaild())
            throw SDLException(
                "Cannot set window title, SDL window is invaild.");
